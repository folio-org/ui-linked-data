// Temporary constant for quickly enabling/disabling a feature that is under implementation
export const ADVANCED_SEARCH_ENABLED = true;
export const SIMPLE_LOOKUPS_ENABLED = true;
export const LOCALE_SELECT_ENABLED = false;
export const SEARCH_FILTERS_ENABLED = false;
export const IS_DISABLED_FOR_ALPHA = true;
export const IS_NEW_SEARCH_ENABLED = true;
export const IS_HUBS_CREATE_ENABLED = false;
